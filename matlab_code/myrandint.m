...
%if (eq(size(outputRange),[1 2]) && ~eq(outputRange(1),outputRange(2)-1))
%    warning('To specify a range [low high] use [low:high].');
%end;
if (~eq(round(outputRange),outputRange))
    warning('Specified RANGE contains noninteger values.');
end;
if (~eq(length(outputRange),length(outputRange(:))))
...
if (isempty(varargin))
    ...
    if (ne(size(randIx),size(ranInt)))
        ranInt = reshape(ranInt,size(randIx));
    end;
    
elseif (eq(varargin{1},'noreplace'))
